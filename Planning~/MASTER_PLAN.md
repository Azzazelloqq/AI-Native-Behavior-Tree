...
Phase 4 (benchmark scheduler research) was decomposed into `P4-001` through `P4-009` on 2026-08-19, grounded directly in `Documentation~/benchmarks.md`'s six-step "Scheduler research" process, `Documentation~/execution-and-scheduling.md`'s policy/work-estimation/batching/explainability contract, and `OQ-006` (`Planning~/OPEN_QUESTIONS.md`). `P4-001` (benchmark scenario catalog and harness) and `P4-003` (`PipelinedJobs` executor) are `Ready` now — `P4-001` depends only on the already-accepted `P2-025`, and `P4-003` depends only on the already-accepted `P2-019`/`P2-020`, so the two can proceed independently and in parallel. `P4-002` (fixed-policy cost curves) through `P4-009` (the Phase 4 gate) remain `Draft`, gated by their own dependency chain: `P4-002` needs `P4-001`; `P4-004` (work-estimation/batching calibration) needs `P4-002`; `P4-005` (`Auto` heuristic selection) needs both `P4-003` and `P4-004`; `P4-006` (`Auto`-vs-fixed comparison) needs `P4-001` and `P4-005`; `P4-007` (`OQ-006`'s autotuning resolution — conditional per `benchmarks.md`'s own step 5/6 gate, not predetermined to ship adaptation) needs `P4-006`; `P4-008` (platform benchmark evidence on the mandatory pre-1.0 targets) needs `P4-005` and `P4-007`; `P4-009` needs all eight. No performance default, regression threshold, or supported-hardware-class claim is authorized by this decomposition itself — every P4 card's own `Forbidden changes` repeats `Planning~/USER_ACTIONS.md`'s requirement that thresholds and hardware-class approval come from the owner after the research exists, not from an implementation agent.
