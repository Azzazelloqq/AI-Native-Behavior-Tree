...
| OQ-007 | Define what "reload" means for a semantically changed tree with a live instance mid-execution (abort and restart? migrate in place? explicitly unsupported for a first cut?), and the compatibility-classification rule the reload strategy depends on. | Resolved, see [ADR P5-001](../Documentation~/decisions/ADR-P5-001-hot-reload-compatibility-model.md) | None |
| OQ-008 | `NativeWorkEstimatorV1.CalibratedNanosecondsPerNodeStep` and the `Immediate`-over-`BatchedJobsSameFrame` preference order (`P6-019`) were both measured with AIBT as the only real consumer of Unity's Job Worker pool. A project also running another heavy Job/Burst-based system on the same pool (e.g. a DOTS-based navigation/crowd system) was never part of that measurement. Does the calibration/preference order still hold under that kind of shared contention, or does it need a dedicated re-measurement methodology (and does `ProductionTreeScheduler`'s own `SetBudgetProvider` need a documented pattern for one shared cross-system frame-budget authority, rather than each system estimating in isolation)? Raised 2026-09-07 from an owner question about coexisting with a DOTS navigation library; not yet investigated. | Unassigned -- needs a dedicated benchmark spike with a real competing Job workload, not just re-running the existing isolated `Benchmarks~/Phase4/` harness | A future card touching multi-system Job coexistence guidance (none scheduled yet) |
