# Proposed ADR: Editor graph framework
...
This decision does **not** select a replacement. Per the `P3-001` card's own
handoff note, rejecting the only-evaluated candidate does not authorize
picking an unevaluated fallback. `AIBT-012` should move from "Pending spike"
to a new "Pending second spike" state (or equivalent), naming a follow-up
spike that evaluates a custom UI Toolkit-based graph view and/or the older
`UnityEditor.Experimental.GraphView` API against the same four criteria
(serialization control, extensibility, large-graph interaction performance —
including actual live-GUI interaction latency, which this spike could not
measure — and testability) before any `P3-002`+ implementation card can
proceed.

## Consequences

- Every `P3` card downstream of `P3-001` remains blocked until a follow-up
  spike accepts a specific framework; none may proceed on the assumption
  that Graph Toolkit is available.
- `Documentation~/editor-and-layout.md`'s manual-organization requirements
  (groups, comments, sticky notes, reroutes) constrain the next candidate
  directly — any framework evaluated next must support them natively or via
  a documented, evaluated extension point, not be assumed compatible.
