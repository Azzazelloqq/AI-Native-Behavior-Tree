...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    ...
    $OutputPath = Join-Path $PSScriptRoot "Results\scheduling-windows-editor-$stamp.json"
}
...
if ([string]::IsNullOrWhiteSpace($IsolatedProjectPath)) {
    $leaf = 'aibt-p4-001-scheduling-benchmark-' + [Guid]::NewGuid().ToString('N')
...
$benchmarkDestination = Join-Path $assetsRoot 'AIBTSchedulingBenchmark'
...
$unityArguments.AddRange([string[]]@(
    ...
    '-executeMethod', 'AIBT.Benchmarks.Phase4.Scheduling.SchedulingBenchmarkRunner.Run',
