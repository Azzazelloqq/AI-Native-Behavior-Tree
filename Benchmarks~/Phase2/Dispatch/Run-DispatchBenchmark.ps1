...
if ($DispatchesPerSample -lt 128 -or $DispatchesPerSample % 128 -ne 0) {
    throw 'DispatchesPerSample must be at least 128 and divisible by 128.'
}
...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    ...
    $OutputPath = Join-Path $PSScriptRoot "Results\dispatch-windows-editor-$stamp.json"
}
...
if ([string]::IsNullOrWhiteSpace($IsolatedProjectPath)) {
    $leaf = 'aibt-p2-012-dispatch-benchmark-' + [Guid]::NewGuid().ToString('N')
...
Copy-Item -LiteralPath $benchmarkSource -Destination (Join-Path $assetsRoot 'AIBTDispatchBenchmark') -Recurse
...
$unityArguments = @(
    ...
    '-executeMethod', 'AIBT.Benchmarks.Phase2.Dispatch.DispatchBenchmarkRunner.Run',
    ...
    '-aibtDispatchesPerSample', $DispatchesPerSample.ToString([Globalization.CultureInfo]::InvariantCulture),
    '-logFile', $logPath,
    '-quit'
)
