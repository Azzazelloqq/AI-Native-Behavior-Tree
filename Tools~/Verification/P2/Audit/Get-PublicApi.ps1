...
if ([string]::IsNullOrWhiteSpace($IsolatedProjectPath)) {
    $IsolatedProjectPath = Join-Path ([IO.Path]::GetTempPath()) ('aibt-p2-025-publicapi-' + [Guid]::NewGuid().ToString('N'))
}
...
# Reflecting over Unity-Mono-compiled netstandard2.1 assemblies from plain
# Windows PowerShell 5.1 cannot reliably resolve netstandard/Unity BCL
# dependencies. Unity itself can always load its own compiled output, so the
# dump runs as an -executeMethod Editor script instead of host-process
# reflection, matching every other build/verification driver in this repo.
